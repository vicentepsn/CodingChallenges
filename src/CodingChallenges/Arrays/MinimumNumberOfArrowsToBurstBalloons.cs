...
/// Groups    : Array, Sorting, Intervals, Greedy??
...
public class MinimumNumberOfArrowsToBurstBalloons
{
    public int FindMinArrowShots(int[][] points)
    {
        ...
        int minArrowShots = points.Length;
        ...
        {
            if (intersectionEnd >= points[i][0])
            {
                intersectionStart = Math.Max(intersectionStart, points[i][0]);
                intersectionEnd = Math.Min(intersectionEnd, points[i][1]);
                minArrowShots--;
            }
            else
            {
                intersectionStart = points[i][0];
                intersectionEnd = points[i][1];
            }
        }
    ...
    {
        int compareStart = a[0].CompareTo(b[0]);

        return compareStart != 0 ? compareStart : a[1].CompareTo(b[1]);
    }
}
/*
